...
export const getMaterials = async (Nombre_tecnico) => {
  try {
    const response = await fetch('http://3.15.96.58/stocktechnique/materials-by-tecnico', {
      method: 'POST',
      headers: {
...
export const getTechnicians = async () => {
  try {
    const response = await fetch('http://3.15.96.58/stocktechnique/all-tecnicos');
    if (!response.ok) {
...
export const getStockByMaterial = async (Nombre_material, Nombre_tecnico) => {
  try {
    const response = await fetch('http://3.15.96.58/stocktechnique/stock-by-name', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
export const SaveStockTecnico = async (Nombre_material, Cantidad, Nombre_tecnico) => {
  try {
    ...
    const responseStockTecnico = await axios.put('http://3.15.96.58/stocktechnique/update-cantidad-stocktechnique', qs.stringify(formDataStockTecnico), {
      headers: {
    ...
    const responseStockSistema = await axios.post('http://3.15.96.58/stock/update-stockbydevolucion', qs.stringify(formDataStockSistema), {
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      }
    });
