...
export const SaveStockTecnico = async (Nombre_material, Cantidad, Nombre_tecnico) => {
  try {
    ...
    const responseStockTecnico = await axios.put('https://legonalmacenfrailes-1.onrender.com/stocktechnique/update-cantidad-stocktechnique', qs.stringify(formDataStockTecnico), {
      headers: {
    ...
    const responseStockSistema = await axios.post('https://legonalmacenfrailes-1.onrender.com/stock/update-stockbydevolucion', qs.stringify(formDataStockSistema), {
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      }
    });
