...
export const SaveTraslado = async (Sede_origen, Sede_destino, Nombre_material, Cantidad) => {
  try {
    ...
    const responseStockTecnico = await axios.post('http://3.15.96.58/traslado/add', qs.stringify(formDataTraslado), {
      headers: {
    ...
    if (Sede_origen === 'Molivento') {
      ...
      responseStockSistema = await axios.post('http://3.15.96.58/stock/update-stockbydevolucion', qs.stringify(formDataStockSistema), {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        }
      });
      ...
      responseStockSistema = await axios.post('http://3.15.96.58/stock/update-stockbytecnico', qs.stringify(formDataStockSistema), {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        }
      });
