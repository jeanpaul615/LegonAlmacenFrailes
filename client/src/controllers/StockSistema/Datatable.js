...
export const fetchStocks = async () => {
  try {
    const response = await axios.get('http://3.15.96.58/stock/get-stocksistema');
...
export const deleteStock = async (id) => {
  const response = await fetch(`http://3.15.96.58/delete-stocksistema/${id}`, {
    method: 'DELETE'
  });
  if (!response.ok) {
...
export const updateStock = async (Id_stocksistema, Nombre_material, Cantidad, Estado) => {
  ...
  try {
    ...
    const response = await fetch(`http://3.15.96.58/stock/update-stocksistema/${Id_stocksistema}/${Nombre_material}`, {
      method: 'PUT',
      headers: {
