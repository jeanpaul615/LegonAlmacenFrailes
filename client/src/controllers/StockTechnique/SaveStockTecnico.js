...
export const SaveStockTecnico = async (Id_stocktecnico, Nombre_material, Cantidad, Nombre_tecnico) => {
  try {
    ...
    const responseStockTecnico = await axios.post('http://3.15.96.58/stocktechnique/add-stocktechnique', qs.stringify(formDataStockTecnico), {
      headers: {
    ...
    const responseStockSistema = await axios.post('http://3.15.96.58/stock/update-stockbytecnico', qs.stringify(formDataStockSistema), {
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      }
    });
